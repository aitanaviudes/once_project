...
class DemoReplayer:
    ...
    def _init_moveit(self):
        ...
        if self.gripper_group is not None:
            ...
            self.gripper_group.set_planning_time(5.0)
        ...
        print(
            "  Gripper group:  "
            f"{'interbotix_gripper' if self.gripper_group is not None else 'not available'}"
        )
    ...
    @staticmethod
    def _extract_gripper_states(eef_twists, expected_len):
        """Extract boolean gripper states from twist array column 6."""
        if eef_twists is None or expected_len <= 0:
            return np.zeros(expected_len, dtype=bool)

        twists = np.asarray(eef_twists)
        if twists.ndim != 2 or twists.shape[1] < 7:
            return np.zeros(expected_len, dtype=bool)

        n = min(expected_len, twists.shape[0])
        states = twists[:n, 6] > 0.5
        if n < expected_len:
            pad_val = bool(states[-1]) if n > 0 else False
            states = np.concatenate([states, np.full(expected_len - n, pad_val, dtype=bool)])
        return states.astype(bool)

    @staticmethod
    def _build_state_segments(states):
        """Split trajectory into [start, end) segments where state is constant."""
    ...
    def _set_gripper_state(self, closed, dry_run=False):
        """Command the gripper state (Closed/Open) if the group is available."""
        target_state = bool(closed)
        if self._last_gripper_state is not None and self._last_gripper_state == target_state:
            return True

        label = "CLOSED" if target_state else "OPEN"
        if dry_run:
            print(f"  [DRY RUN] Gripper -> {label}")
            self._last_gripper_state = target_state
            return True

        if self.gripper_group is None:
            print(f"  Warning: gripper group unavailable; cannot set {label}.")
            self._last_gripper_state = target_state
            return False

        named_target = "Closed" if target_state else "Open"
        success = False
        try:
            self.gripper_group.set_named_target(named_target)
            success = self.gripper_group.go(wait=True)
        except Exception:
            # Fallback to explicit joint values if named target is unavailable.
            target = [0.015, -0.015] if target_state else [0.037, -0.037]
            try:
        ...
        if success:
            ...
            print(f"  Warning: failed to set gripper {label}.")
        return bool(success)

    def _execute_plan_now(self, plan, dry_run=False):
        """Execute a plan without fraction checks."""
        if dry_run:
            print("[DRY RUN] Skipping plan execution.")
            return True
    ...
    def _execute_plan(self, plan, fraction, dry_run=False):
        ...
        success = self._execute_plan_now(plan, dry_run=dry_run)
    ...
    def _execute_chunked_cartesian(self, eef_poses, chunk_size=35, dry_run=False):
        ...
        while start < len(eef_poses) - 1:
            seg_idx += 1
            attempt_chunk = min(chunk_size, len(eef_poses) - start)
            plan = None
            fraction = 0.0
            end = start + 1

            while attempt_chunk >= 2:
                end = min(start + attempt_chunk, len(eef_poses))
                ...
                if plan is not None and len(plan.joint_trajectory.points) >= 2 and fraction >= 0.15:
                    break

                attempt_chunk //= 2

            if plan is None or len(plan.joint_trajectory.points) < 2 or fraction < 0.15:
                print("    Segment planning failed even after reducing chunk size.")
                return False

            ok = self._execute_plan_now(plan, dry_run=dry_run)

            if ok:
                executed_segments += 1
                print(f"    Executed (fraction={fraction:.3f})")
            else:
                print("    Execution failed for this segment; stopping.")
                return False
    ...
    def _execute_segmented_trajectory(
        self,
        eef_poses,
        gripper_states,
        downsample=3,
        ...
        """
        Execute trajectory in gripper-state segments.

        Motion is planned/executed as Cartesian paths to keep continuity.
        Gripper state transitions are applied at segment boundaries.
        """
        if len(eef_poses) < 2:
            print("Error: need at least 2 EEF poses to execute.")
            return False

        indices = self._build_indices(len(eef_poses), max(1, downsample))
        ds_poses = np.array([eef_poses[i] for i in indices])

        states = np.asarray(gripper_states, dtype=bool)
        if len(states) != len(eef_poses):
            n = min(len(states), len(eef_poses))
            if n == 0:
                states = np.zeros(len(eef_poses), dtype=bool)
            else:
                pad = np.full(len(eef_poses) - n, states[n - 1], dtype=bool)
                states = np.concatenate([states[:n], pad])
        ds_states = np.array([states[i] for i in indices], dtype=bool)

        segments = self._build_state_segments(ds_states)
        print(f"\nExecuting {len(segments)} motion segments (gripper-aware)...")

        # Set initial gripper state before moving.
        self._set_gripper_state(ds_states[0], dry_run=dry_run)

        executed_any = False
        for seg_idx, (start, end, closed) in enumerate(segments, start=1):
            seg_poses = ds_poses[start:end]
            if len(seg_poses) < 2:
                # Only a state transition/no motion segment.
                print(f"  Segment {seg_idx}: no motion (single waypoint).")
            else:
                print(
                    f"\nSegment {seg_idx}/{len(segments)}: "
                    f"{len(seg_poses)} poses, gripper={'CLOSED' if closed else 'OPEN'}"
                )

                plan, fraction, _ = self._plan_cartesian(
                    seg_poses, downsample=1, position_only=False
                )

                if plan is None or fraction < self.min_fraction:
                    print("  Segment full-pose short, trying position-only...")
                    plan, fraction, _ = self._plan_cartesian(
                        seg_poses, downsample=1, position_only=True
                    )

                if plan is not None and fraction >= self.min_fraction:
                    ok = self._execute_plan_now(plan, dry_run=dry_run)
                else:
                    print("  Segment still low fraction; using chunked Cartesian fallback...")
                    ok = self._execute_chunked_cartesian(
                        seg_poses,
                ...
                if not ok:
                    print("Error: failed to execute segment; stopping replay.")
                    return False
                executed_any = True

            # Apply gripper transition before next segment.
            if seg_idx < len(segments):
                next_closed = segments[seg_idx][2]
                if next_closed != closed:
                    self._set_gripper_state(next_closed, dry_run=dry_run)
    ...
    def run(self, demo_dir, speed_factor=1.0, downsample=3, dry_run=False, mode="trajectory"):
        ...
        gripper_states = self._extract_gripper_states(demo["eef_twists"], len(eef_poses))
        ...
        eef_for_exec = eef_poses[1:]
        gripper_for_exec = gripper_states[1:]
        if not dry_run:
            if not self._move_to_start_pose(start_pose):
                print("  Continuing without start alignment.")
                eef_for_exec = eef_poses
                gripper_for_exec = gripper_states

        if len(eef_for_exec) < 2:
            print("Error: not enough trajectory poses after start handling.")
            return

        ok = self._execute_segmented_trajectory(
            eef_for_exec,
            gripper_for_exec,
    ...
    def run_with_live_data(self, speed_factor=1.0, dry_run=False, mode="trajectory"):
        ...
        gripper_states = self._extract_gripper_states(self.end_effector_twists, len(eef_poses))
